...
def main():
    ...
    for x in range (2900):
        ...
        date_from_format = ("{0:%Y-%m-%d %H:%M}:00".format(today - relativedelta(minutes=5+5*x)))
        date_to_format = ("{0:%Y-%m-%d %H:%M}:59".format(today - relativedelta(minutes=1+5*x)))
        date_from_format_last = ("{0:%Y-%m-%d %H:%M}:00".format(today - relativedelta(minutes=10+5*x)))
        date_to_format_last = ("{0:%Y-%m-%d %H:%M}:59".format(today - relativedelta(minutes=6+5*x)))
